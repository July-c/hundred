        ...
        superForm: function (option) {
            ...
            $form.validator({
                onValid: function (validity) {
                    $(validity.field).next('.layer-alert').hide();
                },
                /**
                 * 显示错误信息
                 * @param validity
                ...
                onInValid: function (validity) {
                    var $field = $(validity.field)
                        , $group = $field.parent()
                        , $alert = $group.find('.layer-alert');

                    if ($field.data('validationMessage') !== undefined) {
                        // 使用自定义的提示信息 或 插件内置的提示信息
                        var msg = $field.data('validationMessage') || this.getValidationMessage(validity);
                        if (!$alert.length) {
                            $alert = $('<div class="layer-alert layer-alert-danger"></div>').hide().appendTo($group);
                        }
                        $alert.html(msg).show();
                    }
                },
            ...
            if (checkedIds.length === 373) {
                ...
                dataTree.forEach(function (item) {
                    ...
                    if (item.city.length > 0) {
                        ...
                        str += ' (<span class="layer-link-muted">'
